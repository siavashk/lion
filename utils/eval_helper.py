...
def compute_NLL_metric(gen_pcs, ref_pcs, device, writer=None, output_name='', batch_size=200, step=-1, tag=''):
    ...
    n_vis = min(10, len(metrics[k1]))
    subtitles = [['ori', 'gen-%s=%.1fx1e-2;%s=%.1fx1e-2' % (k1, metrics[k1][j]*1e2, k2, metrics[k2][j]*1e2)] for j in range(n_vis)]
    pair_vis(gen_pcs[:n_vis], ref_pcs[:n_vis], titles, subtitles, writer, step=step)
    results = {}

    for k in metrics.keys():
        sorted, indices = torch.sort(metrics[k])
        worse_ten, worse_score = indices[-n_vis:].cpu(), sorted[-n_vis:].cpu()
    ...
    logger.info('best 10: {}', indices[:n_vis])
