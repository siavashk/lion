        ...
        for cate_idx, subd in enumerate(self.subdirs):
            ...
            if True:
                ...
                for mid in all_mids:
                    ...
                    if point_cloud.shape[0] < 15_000:
                        num_missing = 15_000 - point_cloud.shape[0]
                        last_row = point_cloud[-1:]
                        padding = np.repeat(last_row, num_missing, axis=0)
                        point_cloud = np.vstack([point_cloud, padding])
                    elif point_cloud.shape[0] > 15_000:
                        point_cloud = point_cloud[:15000]
